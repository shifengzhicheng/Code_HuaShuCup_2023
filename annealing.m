...
T_min = 0.01; % 设置结束温度
P = t_step .* sum((discrete_temperature - 25));

% 调用模拟退火算法
result = simulated_annealing(initial_solution, initial_temperature, cooling_rate, T_min, P);
d = result(1);
rho_s = result(2);
rho_w = result(3);
theta_s = result(4);
...
fprintf('对应的 k_all 值: %f\n', energy_func(d, rho_s, rho_w, theta_s, theta_w, P));
