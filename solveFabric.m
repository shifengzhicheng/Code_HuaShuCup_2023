...
Q_all = -k_all * A_all / h * t_step .* sum((discrete_temperature - 25));

Q_air = -k_air * A_air / h * t_step .* sum((discrete_temperature - 25));

Q_fabric = Q_all - Q_air;

sub_k_fabric = -Q_fabric / (A_fabric*sum(discrete_temperature - 25)*t_step/h);
